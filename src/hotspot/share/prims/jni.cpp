...
static jint JNI_CreateJavaVM_inner(JavaVM **vm, void **penv, void *args) {
  ...
  if (result == JNI_OK) {
    ...
    *(JNIEnv**)penv = thread->jni_environment();
    ...
    ThreadStateTransition::trans_from_unsafe(thread, _thread_in_vm, _thread_in_native);
  } else {
...
static jint JNICALL jni_DestroyJavaVM_inner(JavaVM *vm) {
  ...
  JavaThread* thread = JavaThread::current();
  ThreadStateTransition::trans_from_safe(thread, _thread_in_native, _thread_in_vm);
