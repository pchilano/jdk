...
void MountUnmountDisabler::start_transition(JavaThread* thread, oop vthread, bool is_mount, bool is_thread_end) {
  ...
  if (notify_jvmti_events()) {
    ...
    if (is_thread_end && JvmtiExport::should_post_vthread_end()) {
      JvmtiExport::post_vthread_end((jthread)vth.raw_value());
    }
  }
  ...
  while (is_start_transition_disabled(thread, vth())) {
  ...
  if (notify_jvmti_events()) {
    if (is_thread_end && thread->jvmti_thread_state() != nullptr) {
      ...
      assert(java_lang_Thread::jvmti_thread_state(vth()) == nullptr, "should be null");
    }
    thread->rebind_to_jvmti_thread_state_of(is_mount ? vth() : thread->threadObj());
  }
...
void MountUnmountDisabler::end_transition(JavaThread* thread, oop vthread, bool is_mount, bool is_thread_start) {
  assert(java_lang_Thread::is_in_VTMS_transition(vthread), "");
  assert(thread->is_in_VTMS_transition(), "");
  Handle vth = Handle(thread, vthread);
  ...
  if (notify_jvmti_events()) {
    ...
    if (should_rebind) {
      ...
      thread->rebind_to_jvmti_thread_state_of(is_mount ? vth() : thread->threadObj());
      OrderAccess::fence();  // dont reorder with clear of transition flags below
    }
  }
  ...
  if (notify_jvmti_events()) {
    ...
    if (is_thread_start) {
      ...
      if (JvmtiExport::should_post_vthread_start()) {
        JvmtiExport::post_vthread_start((jthread)vth.raw_value());
      }
    }
    if (is_mount && JvmtiExport::should_post_vthread_mount()) {
